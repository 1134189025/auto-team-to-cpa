...
def _is_otp_input_visible(page, timeout=500):
    return _find_otp_input(page, timeout=timeout) is not None
...
def _normalize_otp_code(code) -> str:
    digits = "".join(ch for ch in str(code or "") if ch.isdigit())
    return digits or str(code or "").strip()


def _visible_otp_cells(page, timeout=1000):
...
def _find_otp_input(page, timeout=5000):
    cells = _visible_otp_cells(page, timeout=min(timeout, 1500))
    if cells:
        return cells[0]
    try:
        locator = page.locator(_OTP_INPUT_SELECTORS).first
...
def _fill_otp_input(page, code, timeout=5000):
    normalized = _normalize_otp_code(code)
    cells = _visible_otp_cells(page, timeout=min(timeout, 1500))
    if len(cells) >= min(len(normalized), 4):
        logger.info("[Codex] detected %d OTP cells", len(cells))
    ...
    otp_input = _find_otp_input(page, timeout=timeout)
    if otp_input:
        otp_input.fill(normalized)
        return otp_input
    return None


def _mail_item_id(item):
...
def _mail_item_sender(item):
    return str(item.get("sendEmail") or item.get("sender") or item.get("from") or "").lower()


def login_codex_via_browser(email, password, mail_client=None, *, account_id="", workspace_name=""):
    """
    ...
    target_workspace_name = workspace_name or get_chatgpt_workspace_name()
    ...
    with sync_playwright() as p:
        ...
        if mail_client:
            try:
                ...
                if _pre:
                    _email_id_before_login = _mail_item_id(_pre[0])
            except Exception:
                pass
        ...
        code_input = _find_otp_input(page, timeout=5000)
        ...
        if code_input and mail_client:
            ...
            if otp_code:
                ...
                code_input = _fill_otp_input(page, otp_code, timeout=5000)
                if code_input:
                    time.sleep(0.5)
                    _click_primary_auth_button(page, code_input, ["Continue", "Verify"])
                    time.sleep(5)
        ...
        for step in range(10):
            ...
            try:
                otp_input = _find_otp_input(page, timeout=2000)
                if otp_input and mail_client:
                    logger.info(
                    ...
                    if otp:
                        ...
                        for submit_attempt in range(1, 3):
                            ...
                            otp_input = _fill_otp_input(page, otp, timeout=2000)
                            if not otp_input:
                                break
                            time.sleep(0.5)
                            _click_primary_auth_button(page, otp_input, ["Continue", "Verify"])
                            logger.info("[Codex] submitted verification code: %s", otp)
...
class SessionCodexAuthFlow:
    ...
    def _workspace_helper(self):
        from autoteam.chatgpt_api import ChatGPTTeamAPI

        helper = ChatGPTTeamAPI(workspace_name=self.workspace_name)
        helper.page = self.page
        return helper

    def _list_workspace_options(self):
        return self._workspace_helper()._list_workspace_options()

    def _pick_preferred_workspace_option(self, options):
        return self._workspace_helper()._pick_preferred_workspace_option(options)

    def _click_workspace_option_by_label(self, label):
        return self._workspace_helper()._click_workspace_option_by_label(label)

    def _click_workspace_or_consent(self):
        acted = False

        try:
            if "workspace" in self.page.url:
                chosen = self._pick_preferred_workspace_option(self._list_workspace_options())
                if chosen and self._click_workspace_option_by_label(chosen["label"]):
                    self.workspace_name = self.workspace_name or str(chosen.get("label") or "").strip()
    ...
    def submit_code(self, code):
        code_input = _fill_otp_input(self.page, code, timeout=5000)
        if not code_input:
            raise RuntimeError("verification code step is not active")

        time.sleep(0.5)
        _click_primary_auth_button(self.page, code_input, ["Continue", "Verify"])
        time.sleep(5)
        return self._advance()
...
class ParentMainCodexSyncFlow(SessionCodexAuthFlow):
    def __init__(self, *, parent_id, email, session_token, account_id, workspace_name="", password=""):
        ...
        super().__init__(
            ...
            password=password,
            password_callback=None,
            auth_file_callback=lambda bundle: save_parent_main_auth_file(self.parent_id, bundle),
        )
...
def save_parent_main_auth_file(parent_id, bundle):
    """Save a stable main-auth file for a specific parent."""
    ensure_auth_dir()
    filepath = AUTH_DIR / f"codex-main-{str(parent_id or '').strip()}.json"
    return _write_auth_file(filepath, bundle)


def get_saved_parent_main_auth_file(parent_id):
    parent_key = str(parent_id or "").strip()
    if not parent_key:
        return ""
    candidate = AUTH_DIR / f"codex-main-{parent_key}.json"
...
def check_codex_quota(access_token, account_id=None):
    ...
    try:
        ...
        return "check_failed", {"error": str(e)}

    if resp.status_code in (401, 403):
        return "auth_error", {"status_code": resp.status_code, "error": resp.text[:200]}
    ...
    if resp.status_code == 429:
        try:
            ...
            payload = {}
        exhausted_info = get_quota_exhausted_info({}, limit_reached=True) or {}
        exhausted_info.update(
            {
    ...
    if resp.status_code != 200:
        ...
        return "check_failed", {"status_code": resp.status_code, "error": resp.text[:200]}

    try:
        data = resp.json()
    except Exception as exc:
        return "check_failed", {"error": str(exc)}
...
def refresh_access_token(refresh_token):
    ...
    try:
        ...
        logger.error("[Codex] Token refresh request failed: %s", exc)
        raise RuntimeError(f"refresh token request failed: {exc}")

    if resp.status_code != 200:
        logger.error("[Codex] Token 刷新失败: %d", resp.status_code)
        if resp.status_code >= 500 or resp.status_code in {408, 429}:
            raise RuntimeError(f"refresh token transient failure: HTTP {resp.status_code}")
        return None

    try:
        data = resp.json()
    except Exception as exc:
        raise RuntimeError(f"refresh token response parse failed: {exc}")
    return {
